function ConvertTo-Yaml
{
    <#
    .SYNOPSIS
    Creates a YAML representation of the data supplied to the cmdlet

    .DESCRIPTION
    PowerShell has a built in tool for converting data to JSON, but it does not have a cmdlet
    for outputting data as a YAML string.

    This is required for interoperability with other applications, Inspec for example. The cmdlet
    is expected to be used in a pipeline to get data and output as required.

    As this a solely a script based converter it is not meant to be used for large data but for
    small data coversation, e.g. the output of Terraform, this will be adequate.

    .EXAMPLE
    Get-Process wi* | Select-Object Handles,NPM,PM,WS,VM,CPU,Id,ProcessName | ConvertTo-Yaml
    
    .EXAMPLE
    Invoke-Terraform -Output | ConvertTo-Yaml | Out-File -Path inspec_inputs.yml -Append

    .PARAMETER Object
    Object to be represented in YAML. This can be piped to the cmdlet as part of a pipeline or 
    as a paramete
    ...
    Required for formatting. This is used by the cmdlet itself during recursion
