...
NOTE: During development the module is being used as is from a directory in the `github.com/ensono/stacks-dotnet` repo. This is in the `russellseymour/independent-pipeline`` branch.
...
Finally Taskctl was researched and adopted, and later forked to create Eirctl (Ensono Independent Runner Control). This was done because of several factors:
...
- Not only can tasks be defined, but pipelines as well. This means that the different stages can be defined within the configuration and can be called as required by the operator (be it human or a CI/CD agent)
- Eirctl has the ability to produce graphs of the pipelines that have been configured
- The fork allows us to fix issues and add features specific to Ensono's needs
